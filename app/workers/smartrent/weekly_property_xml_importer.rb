...
module Smartrent
  class WeeklyPropertyXmlImporter
    ...
    def self.perform(time)
            ...
            if key == :features
              ...
              if key == :image
                begin
                  ...
                  property.image = image_url if !image_url.blank?
      ...
      if capitol_yard
        Property.unscoped.where("name LIKE '%Capitol Yards%' AND id != #{capitol_yard.id}").update_all({
      ...
      Notifier.system_message("[SmartRent] WeeklyPropertyXmlImporter - SUCCESS", 
        "Executed at #{Time.now}, total_creates: #{total_creates}, total_updates: #{total_updates}, errors: #{errors.length} -- #{errors.join("<br/>")}", ADMIN_EMAIL).deliver_now
