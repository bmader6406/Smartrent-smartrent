module Smartrent
  class ResidentProperty < ActiveRecord::Base
    ...
    after_create do
      if move_in_date.present? and status == Resident.STATUS_CURRENT and property.status == Property.STATUS_CURRENT and (Time.now.differnce_in_months(move_in_date)) >= 1 and (move_out_date.nil? or (move_out_date.differnce_in_months(Time.now.month)) == 1)
